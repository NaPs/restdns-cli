...
class RestdnsCliPrinter(object):
    ...
    def table(self, columns, rows, **kwargs):
        table = prettytable.PrettyTable(columns, **kwargs)
        for k, v in kwargs.iteritems():
            if hasattr(table, k):  # Workaround for a bug in prettytable
                setattr(table, k, v)
        for row in rows:
            table.add_row(row)
        self.p(table.get_string())
    ...
    def edit(self, text):
        ...
        if self._editor is None:
            ...
            return text
        with tempfile.NamedTemporaryFile(mode='w+', suffix='restdns-cli-edit') as ftmp:
